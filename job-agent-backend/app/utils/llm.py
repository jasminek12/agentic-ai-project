...
def call_llm_with_system(*, system: str, user: str, temperature: float = 0.45) -> str:
    """Call Groq with an explicit system message (e.g. prose tasks)."""
    print("[DEBUG] call_llm_with_system invoked")
    api_key = os.getenv("GROQ_API_KEY", "").strip()
    if not api_key:
        raise ValueError("GROQ_API_KEY is not set in environment variables.")

...
function App() {
  ...
  const handleContinueInterviewSession = async (targetSessionId = sessionId) => {
    ...
    try {
      ...
      setDebriefActions(Array.isArray(memory.debrief_actions) ? memory.debrief_actions.slice(0, 3) : [])
      setNextRoundTarget(typeof memory.next_round_target === 'string' ? memory.next_round_target : '')
      setCurriculumPlan(Array.isArray(memory.curriculum_plan) ? memory.curriculum_plan.slice(0, 7) : [])
